...
Note, this microservice is connected to the port 5001.

# How to REQUEST data from the microservice
In order to request data from the microservice, the developer needs to send a HTTP GET request to the endpoint "/api/image" with a query. The full endpoint will probably look like `/api/image?q=image_to_search` where `image_to_search` will be the name of the image (ex. puppy123).
...
`const response = await fetch('http://localhost:5001/api/image?q=${endcodedURIComponent(puppy123)}')`
...
We expect the variable `json_data` to hold the following data:
